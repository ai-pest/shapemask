...
def _rotate_boxes(boxes, n):
  ...
  Args:
    boxes: rank 2 float32 tensor containing the bounding boxes -> [N, 4].
           ...
           between [0, 1].
           Each row is in the form of [ymin, xmin, ymax, xmax].
    n: `int` from 0 to 3. The number of times to rotate masks by 90 
      degrees in anticlockwise direction.

  Returns:
    Rotated boxes.
  """
  n_mod4 = tf.math.floormod(n, 4)

  def rot90(boxes):
    # [1-xmax, ymin, 1-xmin, ymax]
    ymin, xmin, ymax, xmax = tf.split(value=boxes, num_or_size_splits=4, axis=1)
    one_minus_xmax = tf.subtract(1.0, xmax)
    one_minus_xmin = tf.subtract(1.0, xmin)
    return tf.concat([one_minus_xmax, ymin, one_minus_xmin, ymax], 1)

  def rot180(boxes):
    # [1-ymax, 1-xmax, 1-ymin, 1-xmin]
    ymin, xmin, ymax, xmax = tf.split(value=boxes, num_or_size_splits=4, axis=1)
    one_minus_xmax = tf.subtract(1.0, xmax)
    one_minus_xmin = tf.subtract(1.0, xmin)
    one_minus_ymax = tf.subtract(1.0, ymax)
    one_minus_ymin = tf.subtract(1.0, ymin)
    return tf.concat([one_minus_ymax, one_minus_xmax, one_minus_ymin, one_minus_xmin], 1)
  ...
  def rot270(boxes):
    ...
    one_minus_ymax = tf.subtract(1.0, ymax)
    one_minus_ymin = tf.subtract(1.0, ymin)
    return tf.concat([xmin, one_minus_ymax, xmax, one_minus_ymin], 1)

  rotated_boxes = tf.case(
    [(tf.math.equal(n_mod4, 0), lambda: boxes),
     (tf.math.equal(n_mod4, 1), lambda: rot90(boxes)),
     (tf.math.equal(n_mod4, 2), lambda: rot180(boxes)),
     (tf.math.equal(n_mod4, 3), lambda: rot270(boxes)),
    ]
  )
...
def _flip_masks_up_down(masks):
  ...
  Returns:
    flipped masks: rank 3 float32 tensor with shape
      [num_instances, height, width] representing instance masks.
  """
  return tf.reverse(masks, axis=[1])


def _rotate_masks(masks, n):
  """Rotate masks.

  Args:
    masks: rank 3 float32 tensor with shape
      [num_instances, height, width] representing instance masks.
    n: `int` from 0 to 3. The number of times to rotate masks by 
      90 degrees in anticlockwise direction.

  Returns:
    rotated masks: rank 3 float32 tensor with shape
      [num_instances, height, width] representing instance masks.
  """
  # Reference: np.rot90 implementation
  #   (https://github.com/numpy/numpy/blob/v1.19.0/numpy/lib/function_base.py#L59-L143)
  n_mod4 = tf.math.floormod(n, 4)

  rotated_masks = tf.case(
    [(tf.math.equal(n_mod4, 0), lambda: masks),                                                     # Rot0
     (tf.math.equal(n_mod4, 1), lambda: tf.linalg.matrix_transpose(tf.reverse(masks, axis=[2]))),   # Rot90
     (tf.math.equal(n_mod4, 2), lambda: tf.reverse(masks, axis=[1,2])),                             # Rot180
     (tf.math.equal(n_mod4, 3), lambda: tf.reverse(tf.linalg.matrix_transpose(masks), axis=[2])),   # Rot270
    ]
  )
...
def keypoint_flip_vertical(keypoints, flip_point, flip_permutation,
                           scope=None):
  ...
  Returns:
    new_keypoints: a tensor of shape [num_instances, num_keypoints, 2]
  """
  # FIXME: not sure if I did this correctly. Review needed.
  with tf.name_scope(scope, 'FlipVertical'):
    keypoints = tf.transpose(keypoints, [1, 0, 2])
    keypoints = tf.gather(keypoints, flip_permutation)
    v, u = tf.split(value=keypoints, num_or_size_splits=2, axis=1)
    u = flip_point * 2.0 - u
    new_keypoints = tf.concat([v, u], 1)
    new_keypoints = tf.transpose(new_keypoints, [1, 0, 2])
    return new_keypoints
...
def random_vertical_flip(image,
  ...
  with tf.name_scope('RandomHorizontalFlip', values=[image, boxes]):
    ...
    if keypoints is not None and keypoint_flip_permutation is not None:
      ...
      keypoints = tf.cond(
          do_a_flip_random,
          lambda: keypoint_flip_vertical(keypoints, 0.5, permutation),
          lambda: keypoints)
      result.append(keypoints)
...
def random_pi_rotation(image,
                       boxes=None,
                       masks=None,
                       do_half_pi_rotation=False):
  """Randomly rotates the image and detections by (n/2)*pi radian.

  If do_half_pi_rotation is False, pi (180 degrees) rotation will be performed 
  with the probability of 50%. 
  Otherwise, (n/2)*pi (where n=[1..3]) rotation is made. In this case, the 
  probability is 25% for each i in n.

  Args:
    image: rank 3 float32 tensor with shape [height, width, channels].
    boxes: (optional) rank 2 float32 tensor with shape [N, 4]
           containing the bounding boxes.
           Boxes are in normalized form. Their coordinates vary between [0, 1].
           Each row is in the form of [ymin, xmin, ymax, xmax].
    masks: (optional) rank 3 float32 tensor with shape
           [num_instances, height, width] containing instance masks. The masks
           are of the same height, width as the input `image`.
    do_half_pi_rotation: `Bool`. If True, (1/2)*pi and (3/2)*pi rotations will 
           be made.
  ...
  Returns:
    ...
    masks: rank 3 float32 tensor with shape [num_instances, height, width]
           containing instance masks.
  """
  # Rotate by (n/2)*pi radian. Obtains random n (0<=n<=3).
  if do_half_pi_rotation is True:
    n = tf.random.uniform(shape=[], minval=0, maxval=4, dtype=tf.int32)
  else:
    n = tf.random.uniform(shape=[], minval=0, maxval=2, dtype=tf.int32)
    n = tf.math.multiply(n, 2)
  
  with tf.name_scope('RandomPiRotation', values=[image, boxes]):
    result = list()
    ## Rotate the image
    image = tf.cond(n >= 1, 
                    true_fn=lambda: tf.image.rot90(image, k=n),
                    false_fn=lambda: image)
    result.append(image)
    ## Rotate the boxes
    if boxes is not None:
      boxes = tf.cond(n >= 1,
                      true_fn=lambda: _rotate_boxes(boxes, n),
                      false_fn=lambda: boxes)
      result.append(boxes)
    ## Rotate masks
    if masks is not None:
      masks = tf.cond(n >= 1,
                      true_fn=lambda: _rotate_masks(masks, n),
