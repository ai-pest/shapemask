...
def random_vertical_flip(image, boxes=None, masks=None):
  """Randomly flips input image and bounding boxes."""
  results = preprocessor.random_vertical_flip(image, boxes, masks)
  if boxes is None and masks is None:
    return results[0]
  else:
    return results
...
def random_pi_rotation(image, boxes=None, masks=None, 
                       do_half_pi_rotation=False):
  """Randomly rotates the input image and bounding boxes by (n/2)*pi degrees.

  Args:
    image: a Tensor of shape [height, width, 3] representing the input image.
    masks: `Tensor` of shape [N, H, W, 1] representing ground truth masks.
    do_half_pi_rotation: `Bool`. If True, randomly performs 90 or 270 degrees
        rotation. Otherwise, only 180 degrees rotation will be applied.

  Returns:
    masks: `Tensor` of shape [N, H, W, 1] representing the scaled masks.
  """
  results = preprocessor.random_pi_rotation(image, boxes, masks, 
    do_half_pi_rotation=do_half_pi_rotation)
  if boxes is None and masks is None:
    return results[0]
  else:
    return results
