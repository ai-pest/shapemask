...
print('AutoAugment: successfully imported modules. Executing the code.')
...
def apply_matrix_to_single_image(image, matrix):
  ...
  """
  img_dim = tf.shape(image)[0]
  img_dim_even = tf.math.floormod(img_dim, 2)
...
def rotate(image, degrees, replace):
  ...
  ### use tfa instead of contrib_image.
  #image = tfa.image.rotate(wrap(image), radians)
  #return unwrap(image, replace)

  # PR suggested the following method.
  # --------------------------------------------------
  # Rotation Matrix
...
def _rotate_bbox(bbox, image_height, image_width, degrees):
  ...
  rotation_matrix = tf.stack(
      [[tf.reduce_sum(tf.cos(radians)), tf.reduce_sum(tf.sin(radians))],
       [tf.reduce_sum(-tf.sin(radians)), tf.reduce_sum(tf.cos(radians))]])
  print('rotation_matrix')
  print(rotation_matrix)
  print('tf.transpose(coordinates)')
  print(tf.transpose(coordinates))
  new_coords = tf.cast(
      tf.matmul(rotation_matrix, tf.transpose(coordinates)), tf.int32)
  # Find min/max values and convert them back to normalized 0-1 floats.
  min_y = -(tf.cast(tf.reduce_max(new_coords[0, :]), dtype=tf.float32) / image_height - 0.5)
  min_x = tf.cast(tf.reduce_min(new_coords[1, :]), dtype=tf.float32) / image_width + 0.5
  max_y = -(tf.cast(tf.reduce_min(new_coords[0, :]), dtype=tf.float32) / image_height - 0.5)
  max_x = tf.cast(tf.reduce_max(new_coords[1, :]), dtype=tf.float32) / image_width + 0.5
...
def _shift_bbox(bbox, image_height, image_width, pixels, shift_horizontal):
  ...
  if shift_horizontal:
    ...
    min_x = tf.reduce_sum(min_x)
    max_x = tf.reduce_sum(max_x)
  else:
    min_y = tf.maximum(0, min_y - pixels)
    max_y = tf.minimum(image_height, max_y - pixels)
    min_y = tf.reduce_sum(min_y)
    max_y = tf.reduce_sum(max_y)
...
def _shear_bbox(bbox, image_height, image_width, level, shear_horizontal):
  ...
  level = tf.math.reduce_sum(level)
  if shear_horizontal:
  ...
  min_y = tf.cast(tf.reduce_min(new_coords[0, :]), dtype=tf.float32) / image_height
  min_x = tf.cast(tf.reduce_min(new_coords[1, :]), dtype=tf.float32) / image_width
  max_y = tf.cast(tf.reduce_max(new_coords[0, :]), dtype=tf.float32) / image_height
  max_x = tf.cast(tf.reduce_max(new_coords[1, :]), dtype=tf.float32) / image_width
...
def _apply_func_with_prob(func, image, args, prob, bboxes):
  ...
  assert inspect.getfullargspec(func).args[1] == 'bboxes'

  # If prob is a function argument, then this randomness is being handled
  # inside the function, so make sure it is always called.
  if 'prob' in inspect.getfullargspec(func):
    prob = 1.0
