...
try:
  from utils import autoaugment_v2 as autoaugment_utils # pylint: disable=g-import-not-at-top
  print('AutoAugment is imported, nice!')
  autoaug_imported = True
except ImportError:
  print('Failed to improt AutoAugment.')
...
class Parser(object):
  ...
  def __init__(self,
    ...
    Args:
      ...
      aug_rand_hflip: `bool`, if True, augment training with random
        horizontal flip.
      aug_rand_vflip: `bool`, if True, augment training with random
        vertical flip.
      aug_rand_rot: `bool`, if True, augment training with random rotation.
      aug_rand_rot_half_pi: `bool`, if True, allow half-pi (90 or 270 degrees) 
        rotation. Otherwise, only 180 degrees rotation will be performed.
    ...
    self._aug_rand_rot = aug_rand_rot 
    self._aug_rand_rot_half_pi = aug_rand_rot_half_pi 
  ...
  def get_normalized_image(self, image):
    return input_utils.normalize_image(image)
  ...
  def parse_train_data(self, data):
    ...
    # Applies a bunch of preprocessing methods with autoaug.
    image = data['image']
    if self._aug_policy:
    ...
    image = self.get_normalized_image(image)
    ...
    if self._aug_rand_vflip:
      image, boxes, masks = input_utils.random_vertical_flip(
          image, boxes, masks)

    # V2: Optional rotation
    if self._aug_rand_rot:
      image, boxes, masks = input_utils.random_pi_rotation(
          image, boxes, masks, self._aug_rand_rot_half_pi) 
    ...
    print(' - image tensor before resize_and_crop: {}'.format(image))
    ...
    print(' - mask_targets: {}'.format(mask_targets))
    print(' - fine_mask_targets: {}'.format(fine_mask_targets))
    return image, labels
