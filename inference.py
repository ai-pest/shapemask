...
def main(unused_argv):
  ...
  output_html = Path(FLAGS.output_html)
  #output_html.parent.mkdir(parents=True)
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session() as sess:
      ...
      for i, image_file in tqdm(enumerate(image_files), ascii=True, total=len(image_files)):
        logging.debug(' - Processing image %d...' % i)
  ...
  for n in range(len(formatted_image_with_detections_list)):
    formatted_image = formatted_image_with_detections_list[n]
    with io.BytesIO() as stream:
      formatted_image.save(stream, format='JPEG')
      data_uri = base64.b64encode(stream.getvalue()).decode('utf-8')
    image_strs.append(
        '<img src="data:image/jpeg;base64,{}" alt="{}" height=800>'
        .format(data_uri, image_names[n]))
